...
description: Set up and verify a local Laravel 13 development environment, discover or install prerequisites with approval, create a starter-kit application, configure its database, authentication, teams, Laravel Boost, and coding-agent integration, and verify a working development server. Use for new local projects or resuming an incomplete setup; not for production deployment or framework upgrades.
---
